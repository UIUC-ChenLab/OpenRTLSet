...
SRC_DATASET_PATH = "/work/nvme/bcct/lad2025/datasets/mg_verilog_3EV/mg_verilog_3EV.jsonl"
TARGET_DATASET_FOLDER_PATH = "/work/nvme/bcct/lad2025/finetuning/datasets/mgverilog-3EV"
...
def tokenize_function(example, eval=False):
    ...
    for i in range(len(example['output'])):
        _verilog_body_str = example['output'][i]
        _sys_str = example['system'][i]
        _user_str = example['user'][i]
        _user_str = _user_str.strip()
        output_texts.append(
                tokenizer.apply_chat_template(
                    [
                        {"role": "system", "content": _sys_str},
...
def filter_function(example):
    ...
    for i in range(len(example["input_ids"])):
        if len(example["input_ids"][i]) > MAX_SEQ_LENGTH:
            raise RuntimeError("found a sample with sequence length too long with length: "+str(len(example["input_ids"][i])))
